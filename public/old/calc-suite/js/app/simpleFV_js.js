...
function doSimpleFV(IR, annPay, numYears, mngFee, pBOYBalance) {
	...
	console.log('Called doSimpleFV()...');
		...
		for (i = 0; i < idx; i++){
			...
			tableArray.push(yrFromStart, dispNewBal.toFixed(0), IR.toFixed(2), dispAnnPay.toFixed(0), totMngFee.toFixed(0));
			var rcFlag = false;
...
function FVFuncts() {
				...
				this.calculateFVals = function(rcFlag, rowIdx, nextRowIdx) {
					...
					dispNewBal = dispNewBal - totMngFee;
