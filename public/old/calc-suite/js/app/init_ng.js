...
// ================ CalcSuiteApp CONTROLLER ========================== 			
		...
		CalcSuiteApp.controller('CSController', ['$scope', function($scope) {
				...
				$('#WM_Calc .dash').removeClass('collapse'); // wait until default data loads to show calc
		...
		CalcSuiteApp.controller('RORCalcController', ['$scope', function($scope){
				...
				$scope.calcName = "Actual ROR Calculator";
				$scope.BOY = 1000;
				$scope.annPay = 1000;
				$scope.mngFee = 1.00;
			...
			$scope.calculateNow = function() {
				...
				if ( valid === true ) {
					var runSrc = pluginDir + 'inc/ror-calc-results.php';
		...
		$('#WM_Calc #resultsWindow').modal(options);
		}
			...
			$scope.calculateNow = function() {
				...
				function loadCalcResults( runSrc ) {
					...
					// LOAD THE CALC RESULTS PANEL
						...
						// Load the new instance of the panel
							$('#calcResults').load( pluginDir + runSrc, function(){
								//callback
